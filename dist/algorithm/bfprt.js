...
export function bfprt(arr, k) {
    ...
    if (k < small.length)
        return bfprt(small, k);
    else if (k < small.length + same.length)
        return pivot;
    else
        return bfprt(big, k - small.length - same.length);
}
function mom(arr) {
    // biome-ignore lint/style/noNonNullAssertion: index is in bounds
    if (arr.length < 6)
...
export function partition(arr, pivot) {
    const small = [];
    const same = [];
    const big = [];
    arr.forEach((num) => {
